...
public class WheelView extends View {
	...
	private int getItemsWidth() {
		...
		return itemsLayout.getMeasuredWidth();
	}
	...
	private void drawItems(Canvas canvas) {
		...
		int w = itemsLayout.getMeasuredWidth();
		...
		for (int i=0;i<size;i++){
			...
			if (textView != null){
				...
				if (w != 0 && len * textsize >= w){
					textsize = (w-20) / len;
					textView.setTextSize(Tools.px2dip(context, textsize));
				}
			}
		}
	...
	private void addAllView() {
		/*int w = itemsLayout.getMeasuredWidth();
		((ArrayWheelAdapter)viewAdapter).setItemWidth(w);*/
	...
	private boolean addViewItem(int index, boolean first) {
		...
		if (view != null) {
			/*int a = Tools.px2dip(context, itemsLayout.getMeasuredWidth());
