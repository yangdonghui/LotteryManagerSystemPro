...
public class CardSlidePanel extends ViewGroup {
    ...
    private int itemMarginTop = 0; // 卡片距离顶部的偏移量  10
    ...
    private List<SingleTicketBean> dataList; // 存储的数据链表
    ...
    private void orderViewStack() {
        synchronized (obj1) {
            ...
            CardItemView lastView = (CardItemView) viewList.get(viewList.size()-1);
            ...
            // 3. changedView填充新数据
            int newIndex = changedView.getIndex();//isShowing + 4;//4
            if (newIndex < dataList.size()) {
                SingleTicketBean dataItem = dataList.get(newIndex);
                changedView.fillData(newIndex, dataList.size(), dataItem);
     ...
     */
    public void fillData(List<SingleTicketBean> dataList) {
        this.dataList = dataList;

        int num = viewList.size();

        if (viewList.size() < dataList.size()) return;

        for (int i = 0; i < num; i++) {
            CardItemView itemView = viewList.get(i);
            if (itemView != null && i < dataList.size()){
                itemView.fillData(i, num, dataList.get(i));
