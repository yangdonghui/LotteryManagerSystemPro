...
public class SwipeListViewTouchListener implements View.OnTouchListener {
     ...
     */
    public SwipeListViewTouchListener(SwipeListView swipeListView, int swipeFrontView, int swipeBackView, float maxMoveDis) {
        ...
        configShortAnimationTime = 500;//swipeListView.getContext().getResources().getInteger(android.R.integer.config_shortAnimTime);
        animationTime = configShortAnimationTime;
        this.swipeListView = swipeListView;

        this.maxMoveDis = maxMoveDis;
    }
    ...
    public boolean onTouch(View view, MotionEvent motionEvent) {
        ...
        switch (MotionEventCompat.getActionMasked(motionEvent)) {
            case MotionEvent.ACTION_DOWN: {
                if (curPosition != -1){
                    //有打开的item
                    View child = swipeListView.getChildAt(curPosition);
                    setFrontView(child.findViewById(swipeFrontView));
                    generateAnimate(frontView, true, false, curPosition);
                    curPosition = -1;
                    return false;
                }
            ...
            case MotionEvent.ACTION_UP: {
                ...
                } else if (Math.abs(deltaX) > maxMoveDis/*viewWidth / 2*/) {
                    swap = true;
                    swapRight = deltaX > 0;

                    if (swipeMode == SwipeListView.SWIPE_MODE_LEFT && swapRight){
                        swapRight = false;
                    }
                }
...
//                }
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (swiping && downPosition != ListView.INVALID_POSITION) {
                    ...
                    }else if(swipeMode == SwipeListView.SWIPE_MODE_LEFT && deltaX > 0){
                        //未打开 超出右侧
                        generateAnimate(frontView, false, true, downPosition);
                        return true;
