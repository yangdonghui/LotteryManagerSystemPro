...
public class SmoothImageView extends ImageView{
    ...
    //是否可触摸缩放图片
    private boolean isTouchScale = false;
...
//      setBackgroundColor(mBgColor);
    ...
    private class Transfrom {
        ...
        float endScaleX;// 图片结束的缩放值
        float endScaleY;// 图片结束的缩放值
    ...
    private void initTransform() {
        ...
        if (mBitmap.getWidth() >= Tools.px2dip(context, getWidth()) && mBitmap.getHeight() >= Tools.px2dip(context, getHeight())) {
            //宽高都超出屏幕，宽高非等比缩放
            mTransfrom.endScaleX = xEScale;
            mTransfrom.endScaleY = yEScale;
        } else {
            //只有宽或高其中一个超出屏幕，等比例缩放
            mTransfrom.endScaleX = endScale;
            mTransfrom.endScaleY = endScale;
        }
        ...
        float bitmapEndWidth = mBitmap.getWidth() * mTransfrom.endScaleX;// 图片最终的宽度
        float bitmapEndHeight = mBitmap.getHeight() * mTransfrom.endScaleY;// 图片最终的宽度
        mTransfrom.endRect.left = (getWidth() - bitmapEndWidth) / 2 - 50;
        mTransfrom.endRect.top = (getHeight() - bitmapEndHeight) / 2 - 50;
        mTransfrom.endRect.width = bitmapEndWidth+100;
        mTransfrom.endRect.height = bitmapEndHeight+100;
    ...
    private void getBmpMatrix() {
        ...
        /* 下面实现了CENTER_CROP的功能 */
        mSmoothMatrix.setScale(mTransfrom.scaleX, mTransfrom.scaleY);
        mSmoothMatrix.postTranslate(-(mTransfrom.scaleX * mBitmap.getWidth() / 2 - mTransfrom.rect.width / 2),
                -(mTransfrom.scaleY * mBitmap.getHeight() / 2 - mTransfrom.rect.height / 2));
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mState == STATE_TRANSFORM_IN || mState == STATE_TRANSFORM_OUT) {
            ...
            if (mTransformStart) {
                ...
                Log.d("Dean", "mTransfrom.scaleX:" + mTransfrom.scaleX);
    ...
    private void startTransform(final int state) {
        ...
        if (state == STATE_TRANSFORM_IN) {
            ...
            PropertyValuesHolder scaleXHolder = PropertyValuesHolder.ofFloat("scaleX", mTransfrom.startScale, mTransfrom.endScaleX);
            PropertyValuesHolder scaleYHolder = PropertyValuesHolder.ofFloat("scaleY", mTransfrom.startScale, mTransfrom.endScaleY);
            ...
            valueAnimator.setValues(scaleXHolder, scaleYHolder, leftHolder, topHolder, widthHolder, heightHolder, alphaHolder);
        } else {
            //PropertyValuesHolder scaleHolder = PropertyValuesHolder.ofFloat("scale", mTransfrom.endScale, mTransfrom.startScale);
            PropertyValuesHolder scaleXHolder = PropertyValuesHolder.ofFloat("scaleX", mTransfrom.endScaleX, mTransfrom.startScale);
            PropertyValuesHolder scaleYHolder = PropertyValuesHolder.ofFloat("scaleY", mTransfrom.endScaleY, mTransfrom.startScale);
        ...
        valueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public synchronized void onAnimationUpdate(ValueAnimator animation) {
                ...
                mTransfrom.scaleX = (Float) animation.getAnimatedValue("scaleX");
                mTransfrom.scaleY = (Float) animation.getAnimatedValue("scaleY");
