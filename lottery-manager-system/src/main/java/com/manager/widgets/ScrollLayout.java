...
public class ScrollLayout extends ViewGroup {
	...
	private float mLastMotionX, mDownMotionX;
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:{
			...
			mLastMotionX = x;
			float a = Math.abs(getScrollX());

			//两端处理
			if ((deltaX < 0 && a >= getWidth()*0.3 && Math.abs(mDownMotionX - x) >= getWidth()*0.3)
					|| (deltaX > 0 && a >= getWidth()*(getChildCount()-1) + getWidth() * 0.3 && Math.abs(mDownMotionX - x) >= getWidth()*0.3)){
				isFlag = false;
				return false;
			}
