 ...
 */
public class DeclareRecordTabFragment2 extends Fragment {
    ...
    //静态对象
    private static DeclareRecordTabFragment2 mDeclareTabFragment = null;

    /**
     * 静态工厂方法需要一个int型的值来初始化fragment的参数，
     * 然后返回新的fragment到调用者
     */
    public static DeclareRecordTabFragment2 newInstance() {
        if (mDeclareTabFragment == null) {
            mDeclareTabFragment = new DeclareRecordTabFragment2();
        }
    ...
    private void updateTipView() {
        if (mLists != null && mLists.size() > 0){
            ...
            }else if (tipView.getVisibility() != View.VISIBLE){
    ...
    private void InitData() {
        ...
        for(int i=0;i< Constants.DeclareLists.size();i++)
        {
            ...
            if (!att.getDeclareState().equals("已完成") && !att.getDeclareState().equals("未受理")){
